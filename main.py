...
class WeightInput(ctk.CTkFrame):
    ...
    def update_weight(self, info=None):
        ...
        if self.metric_bool.get():
            self.output_string.set(f'{round(self.weight_float.get(), 1)}')
        else:
...
class HeightInput(ctk.CTkFrame):
    def __init__(self, parent, height_int, metric_bool):
        ...
        # widgets
        slider = ctk.CTkSlider(self, button_color=GREEN, button_hover_color=DARK_GREEN, progress_color=GREEN,
                               fg_color=LIGHT_GRAY, variable=height_int, from_=100, to=220, command=self.update_text)
...
class UnitSwitcher(ctk.CTkLabel):
    def __init__(self, parent, metric_bool):
        ...
        self.place(relx=.98, rely=.01, anchor='ne')
